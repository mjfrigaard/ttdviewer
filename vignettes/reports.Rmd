...
`render_quarto_report()` will render the report if the quarto R package is installed, and `render_quarto_system_call()` is called for command-line installations. The `data_list` is always passed via an RDS file rather than directly through `execute_params` to avoid the `check_params_for_na()` validation that rejects data frames with `NA` values.
