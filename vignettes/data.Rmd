...
The `.tt` attribute also has `.files`, `.readme`, and `.date` attributes.
...
The `get_tt_tbl()` function allows users to download a "*`tibble` of information about TidyTuesday weeks in the requested timeframe*" 
```{r}
#| label: get_tt_tbl
