...
# Load data_list from RDS path (Quarto) or directly from params (rmarkdown)
data_list <- if (!is.null(params$data_list_path) && nzchar(params$data_list_path) &&
                   file.exists(params$data_list_path)) {
  readRDS(params$data_list_path)
} else if (!is.null(params$data_list) && length(params$data_list) > 0) {
  params$data_list
} else {
  list()
}
...
if (!is.null(data_list) && length(data_list) > 0) {
  ...
  cat("**Number of datasets:**", length(data_list), "\n\n")
...
plot_labels <- c(
  ...
  imb   = "feature imbalance",
  num   = "numeric distributions",
  cat   = "categorical frequencies"
)
chosen <- intersect(params$plot_type, names(plot_labels))
plot_description <- if (length(chosen) == 0) {
  "data overview"
} else {
  paste(plot_labels[chosen], collapse = ", ")
}
```
...
```{r plots}
#| fig-cap: !expr paste("Data visualization:", paste(params$plot_type, collapse = ", "))
