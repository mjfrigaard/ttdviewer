...
mod_plot_server <- function(id, ttd) {
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    logr_msg(sprintf("mod_single_plot_server[%s]: initializing", id), level = "INFO")

    # wrap static into reactive if needed
    ttd_r <- if (is.reactive(ttd)) {
      logr_msg("ttd is reactive", level = "DEBUG")
      ttd
    } else {
      logr_msg("ttd is static; wrapping in reactive()", level = "DEBUG")
      reactive(ttd)
    }
    ...
    observe({
      tryCatch({
        ...
        if (is.null(ds) || length(ds) == 0) {
          ...
          logr_msg("No names in ttd; using indices", level = "WARN")
        }
        updateSelectInput(
          session, "dataset",
          choices  = ds,
        ...
        logr_msg(sprintf("Populated dataset selector with: %s", paste(ds, collapse = ", ")),
                 level = "SUCCESS")
      }, error = function(err) {
        logr_msg(sprintf("Error populating dataset selector: %s", err$message),
                 level = "ERROR")
      })
    }) |>
    ...
    output$plots_ui <- renderUI({
      tryCatch({
        ...
        logr_msg(sprintf("Error in plots_ui renderUI: %s", err$message),
                 level = "ERROR")
        tagList(
    ...
    observe({
        tryCatch({
          ...
          df <- ds_list[[sel_ds]]
          logr_msg(sprintf("Rendering plots for dataset '%s'", sel_ds),
                   level = "INFO")
          ...
          purrr::walk(input$plots, function(plt) {
            ...
            output[[out_id]] <- renderPlot({
              tryCatch({
                inspect_plot(
                  setNames(list(df), sel_ds),
                  plot = plt
                )
              }, error = function(err2) {
                logr_msg(sprintf("Error plotting %s:%s — %s",
          ...
          logr_msg(sprintf("Error in plot observer: %s", err$message),
                   level = "ERROR")
        })
      }) |>
    ...
    logr_msg(sprintf("mod_single_plot_server[%s]: setup complete", id), level = "SUCCESS")
  })
}
