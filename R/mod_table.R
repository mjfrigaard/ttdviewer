...
mod_table_server <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    logr_msg(message = "Initializing table server module", level = "DEBUG")

    observe({
      req(data())
      ds_names <- names(data())
      updateSelectInput(inputId = "ds_input", choices = ds_names, selected = ds_names[1])
    }) |>
      bindEvent(data())
    ...
    observe({
      ...
      output$table <- reactable::renderReactable({
        tryCatch({
            req(data())
            if (length(data()) == 0) {
              logr_msg(message = "No data available for table rendering", level = "WARN")
              return(NULL)
            }
            ds_list     <- data()
            ...
            logr_msg(
              message = paste("Rendering table for file:", ds_selected,
                "with", nrow(display_ds), "rows and", ncol(display_ds), "columns"),
              level = "INFO")

            if (!is.data.frame(display_ds)) {
              logr_msg(message = "Selected dataset is not a data frame", level = "ERROR")
              return(NULL)
            }

            display_ds <- if (nrow(display_ds) > 1000) {
              logr_msg(message = "Large dataset detected - limiting to first 1000 rows", level = "INFO")
          ...
          error = function(e) {
            logr_msg(message = paste("Error rendering table:", e$message), level = "ERROR")
            reactable::reactable(
              data.frame(Error = paste("Failed to load data:", e$message)),
              searchable = FALSE, sortable = FALSE, pagination = FALSE
            )
          })
      })
    }) |>
